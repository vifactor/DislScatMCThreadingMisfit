...
std::ostream& operator<<(std::ostream& out, const Geometry::Vector3d& vec)
{
...
void DislocationPerpendicular::moveTo(const Geometry::Vector2d& pos)
{
    m_pos.set(pos, 0.0);
}
...
void DislocationPerpendicular::setup(const Geometry::Vector3d& Burgers, const Geometry::Vector3d& normal)
{
	...
	m_axis_x = Burgers - m_axis_z * (Burgers * m_axis_z);
	if(m_axis_x.norm2() > 0)
	{
		m_axis_x.normalize();
	}
	...
	m_bx = Burgers * m_axis_x;
	m_bz = Burgers * m_axis_z;
